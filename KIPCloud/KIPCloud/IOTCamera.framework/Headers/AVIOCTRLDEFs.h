...
typedef enum 
{
	...
	IOTYPE_USER_IPCAM_START_RESP				= 0x01FE,
	IOTYPE_USER_IPCAM_STOP	 					= 0x02FF,
	IOTYPE_USER_IPCAM_STOP_RESP					= 0x02FE,
	IOTYPE_USER_IPCAM_AUDIOSTART 				= 0x0300,
	IOTYPE_USER_IPCAM_AUDIOSTART_RESP			= 0x0302,
	IOTYPE_USER_IPCAM_AUDIOSTOP 				= 0x0301,
	IOTYPE_USER_IPCAM_AUDIOSTOP_RESP			= 0x0303,

	IOTYPE_USER_IPCAM_SPEAKERSTART 				= 0x0350,
	IOTYPE_USER_IPCAM_SPEAKERSTART_RESP			= 0x0352,
	IOTYPE_USER_IPCAM_SPEAKERSTOP 				= 0x0351,
	IOTYPE_USER_IPCAM_SPEAKERSTOP_RESP			= 0x0353,
	IOTYPE_USER_IPCAM_SPEAKER_CONNECT_STATUS	= 0x0354,
	...
	IOTYPE_USER_IPCAM_SETAP_REQ					= 0x034A,
	IOTYPE_USER_IPCAM_SETAP_RESP				= 0x034B,
	IOTYPE_USER_IPCAM_GETAP_REQ					= 0x034C,
	IOTYPE_USER_IPCAM_GETAP_RESP				= 0x034D,
	...
	IOTYPE_USER_IPCAM_SELECTSTREAM_REQ			= 0x036A,
	IOTYPE_USER_IPCAM_SELECTSTREAM_RESP			= 0x036B,

	IOTYPE_USER_IPCAM_SET_ENCOPT_REQ			= 0x036C,
	IOTYPE_USER_IPCAM_SET_ENCOPT_RESP			= 0x036D,
	IOTYPE_USER_IPCAM_GET_ENCOPT_REQ			= 0x036E,
	IOTYPE_USER_IPCAM_GET_ENCOPT_RESP			= 0x036F,
	...
	IOTYPE_USER_IPCAM_GET_VERSION_REQ 			= 0x0400,	// Get Firmware Version
	IOTYPE_USER_IPCAM_GET_VERSION_RESP 			= 0x0401,	
	 
	IOTYPE_USER_IPCAM_UPGRADE_VERSION_REQ 		= 0x0402,	// Firmware Upgrade Request
	IOTYPE_USER_IPCAM_UPGRADE_VERSION_RESP 		= 0x0403,

	IOTYPE_USER_IPCAM_GET_DROPBOX_REQ 			= 0x0500,
	IOTYPE_USER_IPCAM_GET_DROPBOX_RESP 			= 0x0501,
	IOTYPE_USER_IPCAM_SET_DROPBOX_REQ 			= 0x0502,
	IOTYPE_USER_IPCAM_SET_DROPBOX_RESP 			= 0x0503,

	/* RF module ref */	
	IOTYPE_USER_IPCAM_ADDDELRF_REQ				= 0x0600,
	IOTYPE_USER_IPCAM_ADDDELRF_RESP				= 0x0601,

	IOTYPE_USER_IPCAM_QUERYRFDEV_REQ			= 0x0602,
	IOTYPE_USER_IPCAM_QUERYRFDEV_RESP			= 0x0603,

	IOTYPE_USER_IPCAM_CONTRLRF_REQ				= 0x0604,
	IOTYPE_USER_IPCAM_CONTRLRF_RESP				= 0x0605,

	IOTYPE_USER_IPCAM_RFALARM_REPORT			= 0x0606,
	IOTYPE_USER_IPCAM_RFALARM_REPORT_RESP		= 0x0607,

	IOTYPE_USER_IPCAM_RF_USERDEFINE_REQ			= 0x0608,
	IOTYPE_USER_IPCAM_RF_USERDEFINE_RESP		= 0x0609,
	...
	IOTYPE_USER_IPCAM_FORCE_IFRAME				= 0x1003,
	
	IOTYPE_USER_IPCAM_ICLOUDCAM_REQ				= 0x1010,	// Get iCloudCam
	IOTYPE_USER_IPCAM_ICLOUDCAM_RESP			= 0x1011,	// iCloudCam Response
...
// AVIOCTRL Event Status Type
typedef enum
{
	AVIOTC_EVENT_RECORD_NOREAD			= 0x00,
	AVIOTC_EVENT_RECORD_READ			= 0x01,
	AVIOTC_EVENT_NORECORD_NOREAD		= 0x02,
	AVIOTC_EVENT_NORECORD_READ			= 0x03,
}ENUM_EVENT_STATUS;
...
/** @struct SMsgAVIoctrlAVStreamResp */
typedef struct
{
	unsigned int channel; // Camera Index
	unsigned char result;	// 0 - successfully, other -failed
	unsigned char reserved[3];
} SMsgAVIoctrlAVStreamResp;

typedef enum {
	ENUM_SPEAKER_CONNECT_OK 	= 0,
	ENUM_SPEAKER_BUSY 			= 1,
	ENUM_SPEAKER_IDLE	 		= 2,
	ENUM_SPEAKER_CONNECT_FAILED = 3,	
} ENUM_SPEAKER_CONNECT_STATUS;
/** @struct SMsgAVIoctrlSpeakerConnectStatus */
typedef struct
{
	unsigned int channel; // Camera Index
	unsigned char result;	// 0 - successfully, other -failed
	unsigned char reserved[3];
} SMsgAVIoctrlSpeakerConnectStatus;
...
/*
IOTYPE_USER_IPCAM_SETAP_REQ					= 0x0360,
** @struct SMsgAVIoctrlSetAPReq
*/
typedef struct
{
	unsigned char ssid[32];			//WiFi ssid
	unsigned char password[64];		//if exist, ap password
	unsigned char enable;			//enable: 1; disable 0
	unsigned char channel;			//channel: 0-aut0, 1-13
	unsigned char enctype;			//refer to ENUM_AP_ENCTYPE
	unsigned char reserved[9];
}SMsgAVIoctrlSetAPReq;
/*
IOTYPE_USER_IPCAM_SETAP_RESP				= 0x0361,
** @struct SMsgAVIoctrlSetAPResp
*/
typedef struct
{
	int result; //0: wifi connected; 1: failed to connect
	unsigned char reserved[4];
}SMsgAVIoctrlSetAPResp;
	

/*
IOTYPE_USER_IPCAM_GETAP_REQ					= 0x0362,
** @struct SMsgAVIoctrlGetAPReq
*/
typedef struct
{
	unsigned char reserved[4];
}SMsgAVIoctrlGetAPReq;

/*
IOTYPE_USER_IPCAM_GETAP_RESP				= 0x0363,
** @struct SMsgAVIoctrlGetAPResp //if no wifi connected, members of SMsgAVIoctrlGetWifiResp are all 0
*/
typedef struct
{
	unsigned char ssid[32];			//WiFi ssid
...
/*
IOTYPE_USER_IPCAM_SELECTSTREAM_REQ			= 0x036A,
** @struct SMsgAVIoctrlSelectStreamReq
*/
typedef struct
{
	unsigned int channel;	// Camera Index
	unsigned char stream;	// 0 - Main stream, 1 - Sub stream, ...
	unsigned char reserved[3];
}SMsgAVIoctrlSelectStreamReq;
/*
IOTYPE_USER_IPCAM_SELECTSTREAM_RESP			= 0x036B,
** @struct SMsgAVIoctrlSelectStreamResp
*/
typedef struct
{
	unsigned int channel; 		// Camera Index
	...
	unsigned char result;		// 0: success; otherwise: failed.
	unsigned char reserved[2];
}SMsgAVIoctrlSelectStreamResp;

/*
IOTYPE_USER_IPCAM_SET_ENCOPT_REQ			= 0x036C,
** @struct SMsgAVIoctrlSetEncOptReq
IOTYPE_USER_IPCAM_GET_ENCOPT_RESP			= 0x036F,
...
typedef struct
{
	unsigned int channel;	// Camera Index
	unsigned char stream;	// 0 - Main stream, 1 - Sub stream, ...
	unsigned char cbrvbr;	// 0 - CBR, 1 - VBR
	unsigned char quality;  // 0 - BEST, 1 - BETTER, 2 - GOOD, 3 - NORMAL, 4 - COMMON
	unsigned char frame;	// 1-30fps.
	unsigned short bitrate;	// kbps.
	unsigned char reserved[2];
}SMsgAVIoctrlSetEncOptReq, SMsgAVIoctrlGetEncOptResp;

/*
IOTYPE_USER_IPCAM_SET_ENCOPT_RESP			= 0x036D,
** @struct SMsgAVIoctrlSetEncOptResp
*/
typedef struct
{
	unsigned int channel; 	// Camera Index
	...
	unsigned char result;	// 0: success; otherwise: failed.
	unsigned char reserved[2];
}SMsgAVIoctrlSetEncOptResp;

/*
IOTYPE_USER_IPCAM_GET_ENCOPT_REQ			= 0x036E,
** @struct SMsgAVIoctrlGetEncOptReq
*/
typedef struct
{
	unsigned int channel; 	// Camera Index
